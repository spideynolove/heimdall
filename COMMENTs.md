...
Add `~/.local/bin` to `PATH` if not already present. This check assumes `bash`:
...
1. Precondition 2 now uses `uv venv`, `source .venv/bin/activate`, and `uv pip install pytest-cov`
2. All steps now start from `cd /path/to/temp-2 && source .venv/bin/activate`
3. Command examples use relative project paths such as `src/ai_profile.py` and `scripts/scaffold-profiles.py`
4. The scaffold step uses `python3` explicitly to avoid Python 2 ambiguity
5. Cleanup now includes `deactivate` so the shell is restored after the test run
